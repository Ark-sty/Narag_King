...
잡기 입력은 0.12초 동안 보관되며, 그동안 장애물의 좌/우 옆면이나 경사면 손잡이의 판정 범위에 들어오면 잡습니다. 잡기 키를 계속 누르고 있는 것만으로는 다음 지점을 자동으로 잡지 않습니다. 경사면의 수직 손잡이는 물리 충돌 없이 잡기 판정만 제공하며, 성공하면 기존 잡기 상태로 전환합니다.
...
맵은 `Game.gd`의 `_build_world()`에서 5개 세로 구간으로 생성됩니다. 충격 피해는 `ImpactDamage.gd`의 `SAFE_IMPACT_SPEED`, `LETHAL_IMPACT_SPEED`, 피해 곡선 상수로 조정합니다. 잡기 거리 `GRAB_SIDE_DISTANCE`, `GRAB_EDGE_DISTANCE`, 입력 버퍼 `GRAB_INPUT_BUFFER_MSEC`와 경사면별 손잡이 판정 범위는 `Game.gd`에서 조정할 수 있습니다.
